...
var _getSortedDocument = function(db, query, sortCondition, callback){
	var arrgegateCommand = _getAggregateCommand(query, sortCondition);
	db.collection(colName).aggregate(arrgegateCommand).toArray(function(err, result) {
		if(err) {
			callback(err, null);
			return;
		}
...
function _getAggregateCommand(query, sortCondition) {
	var sortCondition = 'guides.' + sortCondition;
	var sort = {};
	sort[sortCondition] = -1;
	var arrgegateCommand = [{$match:  query}, {$unwind: '$guides'}, {$sort: sort},{$group: {_id: 0, name: {$first: '$name'}, hf_id:{$first:'$hf_id'}, guides: {$push:'$guides'}}} ];
	if (query == null) {
		arrgegateCommand = [{$match: {}},{$unwind: '$guides'}, {$sort: sort}, {$project: {'name':-1, '_id': 0, 'hf_id':1, 'guide':'$guides'}} ]
	}
	return arrgegateCommand;
}

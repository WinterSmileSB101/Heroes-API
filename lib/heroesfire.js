...
var HeroesFireDB = require('../lib/dataAccess/fireDataAccess');
...
//all guide url for one hero
exports.guidesEachHero = function (req, res, next) {

    //get hero id by name
    var id = heroIdCol[req.query.name];

    guidePagingUrl(id, function (data) {
	    res.json(data);
	});
...
exports.updateHeroIdCol = function (req, res, next) {
    ...
    getHeroID(GLOBAL.URL.HF.guides, function (data) {
        ...
        for (var i = 0; i < len; i++) {
            heroesFireDB.update({ hf_id: data[i]['id'] }, { name: data[i]['name'], hf_id: data[i]['id'] });
        }

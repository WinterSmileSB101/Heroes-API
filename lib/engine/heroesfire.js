...
exports.updateAllHeroGuides = function(req, res, next) {
	heroesFireDB.getAll({}, function(err, result){
		//var result = result.map(function(x) {return x.hf_id;});
		var result = result.map(x=>x.hf_id);
		async.map(result, function(id, callback){
			_getAllGuideUrl(id, function(err, data){
				heroesFireDB.update({hf_id: id}, {guides: data}, function(){
					console.log('--Logging-- guides of hero#'  + id + 'has been updated');
				});
			});
		}, function(err, result){
			if (err) return next(err);
			res.json('All the guides have been updated.')
		});
	});
}
